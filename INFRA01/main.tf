module "module_resource_group" {
  ...
  child_resource_group_name = var.root_resource_group_name
...
module "module_virtual_network" {
  ...
  child_resource_group_name = var.root_resource_group_name
  child_resource_location   = var.root_resource_location
  child_vnet_name           = var.root_vnet_name
...
module "module_subnet" {
  ...
  child_resource_group_name     = var.root_resource_group_name
  child_vnet_name               = var.root_vnet_name
  child_subnet_name             = var.root_subnet_name
...
module "module_nsg" {
  ...
  child_resource_group_name = var.root_resource_group_name
  child_nsg_name            = var.root_nsg_name
  child_resource_location   = var.root_resource_location
}
...
module "module_public_ip" {
  ...
  child_resource_group_name = var.root_resource_group_name
  child_public_Ip_name      = var.root_public_Ip_name
  child_resource_location   = var.root_resource_location
}
...
module "module_nic" {
  ...
  child_resource_group_name = var.root_resource_group_name
  child_resource_location   = var.root_resource_location
  child_nic_name            = var.root_nic_name
  child_ip_config_name      = var.root_ip_config_name
  child_public_Ip_name      = var.root_public_Ip_name
  child_subnet_name         = var.root_subnet_name
  child_vnet_name           = var.root_vnet_name
}
...
module "module_virtual_machine" {
  ...
  child_resource_group_name      = var.root_resource_group_name
  child_resource_location        = var.root_resource_location
  child_virtual_machine_name     = var.root_virtual_machine_name
  child_virtual_machine_username = var.root_virtual_machine_username
  child_virtual_machine_password = var.root_virtual_machine_password
  child_nic_name                 = var.root_nic_name
}

module "module_sql_sqerver" {
  depends_on                = [module.module_virtual_machine]
  source                    = "../modules/09-azurerm_mssql_server"
  child_resource_group_name = var.root_resource_group_name
  child_resource_location   = var.root_resource_location
  child_sql_server_name     = var.root_sql_server_name
  child_sql_server_username = var.root_sql_server_username
  child_sql_server_password = var.root_sql_server_password
}

module "module_sql_database" {
  depends_on                = [module.module_sql_sqerver]
  source                    = "../modules/10-azurerm_mssql_database"
  child_sql_database_name   = var.root_sql_database_name
  child_sql_server_name     = var.root_sql_server_name
  child_resource_location   = var.root_resource_location
  child_resource_group_name = var.root_resource_group_name
}
